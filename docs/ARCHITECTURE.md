...
**Password reset is implemented**, via Resend. `PasswordResetToken` (`prisma/schema.prisma`) is deliberately a separate model from `Session`, not a repurposed one: a reset token is single-use and short-lived (30 minutes, `lib/auth/passwordReset.ts`), where a session is long-lived and reusable — `consumePasswordResetToken` deletes the row the moment it's read, whether or not the caller goes on to actually change the password, so a link can never be replayed even if the request fails partway. Same hashed-at-rest pattern as `Session`: the raw token only ever exists in the emailed link.

`requestPasswordReset` (`lib/actions/passwordReset.ts`) returns the identical message whether or not the email is registered — confirming or denying a registered email is exactly what this endpoint must never leak. `lib/email/resend.ts` fails gracefully like the AI import's missing key: with no `RESEND_API_KEY`, it logs the reset link to the server console instead of emailing it, so local dev works without a Resend account (verified this way — no real key in this environment). `APP_BASE_URL` must be set to the real deployed URL in production, or reset emails link to `localhost`. `/forgot-password` and `/reset-password` had to be added to `proxy.ts`'s `PUBLIC_ROUTES`, same as `/login`/`/signup` — a user resetting their password is by definition logged out.
...
`/import`'s "Adjust existing plan" mode lets the AI modify tasks that already exist, not just create new ones — requested after using AI import in practice and finding no way to say "push this plan back a week" without redoing it by hand. This is a materially different trust problem than import: import only ever creates rows, so the worst a bad parse can do is produce garbage the user declines to commit; adjustment can update or delete a user's existing data, so the design leans harder on validation.

`src/lib/ai/adjust.ts` defines `AdjustmentPlanSchema`: `updates` (each keyed by an existing task's id, only the changed fields set), `deletes` (a list of existing task ids), and `creates` (reusing import's `PlannedDaySchema` — a genuinely new day/task the instruction asks for isn't an "update" to anything). `parseAdjustment` (`src/lib/actions/aiAdjust.ts`) resolves which goal's tasks are in scope — a selected YEAR-level goal plus every descendant (`collectGoalIds`, since a flat AI-imported goal links tasks directly but a hand-built year→month→week hierarchy links them to the leaf week goals), serializes those tasks' real ids/dates/content as context, and instructs the model to reference only those exact ids.

**The model's returned ids are never trusted as-is.** Both when building the preview (`parseAdjustment`) and again independently at commit time (`commitAdjustment`), every `taskId` in `updates`/`deletes` is checked against the actual set of task ids belonging to the current user and the selected goal's scope — anything else (a hallucinated id, or one that's real but outside scope) is silently dropped rather than acted on. This is the same "don't trust model output structurally, verify it against the database" posture as the id-echo pattern generally implies: an id round-tripped through an LLM is a claim, not a fact.

Moving a task to a new date (`newDate` in an update) resolves/creates the target day via `getOrCreateDay` and resets `dailyPriorityRank` to null first — a rank from the old day has no meaning on the new one, mirroring `rescheduleTaskToDate`'s existing behavior for manual drag-and-drop moves in `lib/actions/weeklyReview.ts`. An explicit `isPriority: true` on the same update re-assigns a slot afterward against the *new* day's occupancy, still capped at 3.

The day/task-creation write path itself (get-or-create week/day, respect the Top-3 cap, preserve sort order) was extracted out of `commitParsedPlan` into `src/lib/ai/applyPlan.ts`'s `applyPlannedDays`, so import's full-plan creation and adjustment's `creates` array share one write path instead of two copies that could drift.

**Adjustment scope is any goal level, not just the top-level plan, because scope size is the main lever against slowness.** `fetchAdjustableGoals` lists YEAR, MONTH, *and* WEEK goals that have tasks (directly or via descendants), labeled by level in the dropdown — a hand-built year→month→week hierarchy can be adjusted one week at a time instead of only as a whole. This exists because a content-rewriting instruction (as opposed to a pure date shift) has no compact representation the way `shiftAllByDays` gives a blanket shift: the model genuinely has to reconsider every task in scope, so generation time scales with task count. Confirmed directly: the same restructuring instruction took ~170s and crashed (see below) against a ~103-task plan, but completed in ~44s against a 22-task week once scoped down.

**A blanket date shift is a first-class field, not an enumerated update, because a real plan can be too big to enumerate.** `AdjustmentPlanSchema.shiftAllByDays` lets the model express "push the whole plan back a week" as one number instead of one `updates` entry per task — load-bearing, not an optimization: confirmed directly against a real 252-task plan that without it, the model enumerated all 252 updates, took 167s, and failed validation outright once `updates` exceeded its own cap. `parseAdjustment` expands `shiftAllByDays` into a per-task `newDate` for every task the model didn't already give its own `updates`/`deletes` entry to, entirely server-side — the model never sees or produces the per-task list for the common case. `commitAdjustment` mirrors the scale concern on the write side: target days for a batch of updates are resolved once per distinct date rather than once per task (a shift touches far fewer distinct dates than tasks), deletes go through a single `deleteMany`, and the whole write runs in one transaction with an explicit 60s timeout — Prisma's default interactive-transaction timeout is 5s, which a few hundred sequential per-task updates could plausibly exceed.
